...
<img src="https://img.shields.io/badge/Version-v0.1.0-a855f7?style=flat-square&logo=github" alt="Version"/>
...
**v0.1.0 已完成：** 真实多模型 AI 接入（ernie 系列 / DuckDuckGo），系统提示词自定义，60秒超时保护，一键启停脚本，数据本地存储。
...
| 厂家 | 模型 | 提供方 |
|------|------|--------|
| 🟦 **百度** · `baidu-2api` | ernie-4.0 / ernie-3.5 / ernie-speed / ernie-lite / ernie-tiny | 反代（免费） |
| 🟧 **DuckDuckGo** · `DDG2API` | DuckDuckGo AI | 反代（免费） |
| 🟪 **自定义** · OpenAI 兼容 | 任意兼容模型（DeepSeek / GPT-4o 等） | 用户自配 |

**多源 AI 聚合**：内置百度 ernie 系列与 DuckDuckGo AI 反代，支持用户添加任意 OpenAI 兼容 API。
...
| **AI 反代** | `baidu-2api` · `DDG2API` | ernie + DuckDuckGo 免费 AI |
| **HTTP 客户端** | `requests` | 简洁的同步请求库 |
| **前端** | 原生 HTML / CSS / JS | Gemini 风格 · 暗 / 亮主题 |
| **跨平台启动** | `start.sh` / `start.ps1` | 一键启动，自动检测环境 |
...
│   │   └── ai.py                # AI 对话与模型列表接口
│   └── services/
│       └── ai_service.py        # 核心：百度 / DDG / 自定义模型调度
├── third_party/
│   ├── baidu-2api/              # ernie 反代（端口 8000）
...
| **v0.0.3** | 错误点 AI 自动生成 + 状态格式化 | ✅ 完成 |
| **v0.1.0** | 系统提示词 + 超时保护 + 一键启停 + 模型广场优化 | ✅ 当前版本 |
| **v0.2.0** | 用户系统 + 数据隔离 | 📋 规划 |
| **v0.3.0** | 周 / 月自动总结报告 | 📋 规划 |
...
| 项目 | 说明 |
|------|------|
| [baidu-2api](https://github.com/dijiaozhibei-top/baidu-2api) | ernie 反代 |
